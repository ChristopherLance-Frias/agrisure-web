          ...
          >
            ...
            <td>
              <span class="season-pill">
                {{ claim.damage_report?.season?.name || 'Unknown' }}
              </span>
            </td>
        ...
        <div class="modal">
          <div class="modal-header">
            <div>
              ...
              <p>
                {{ selectedClaim.damage_report?.farm?.farm_name || 'Farm' }}
                •
                {{ selectedClaim.damage_report?.farm?.crop_type || 'Crop' }}
              </p>
            </div>
          ...
          <div class="details-grid">
            ...
            <div class="info-card">
              ...
              <div class="info-row">
                <span>Farm</span>
                <strong>{{ selectedClaim.damage_report?.farm?.farm_name || '—' }}</strong>
              </div>

              <div class="info-row">
                <span>Crop</span>
                <strong>{{ selectedClaim.damage_report?.farm?.crop_type || '—' }}</strong>
              </div>

              <div class="info-row">
                <span>Area</span>
                <strong>{{ selectedClaim.damage_report?.farm?.farm_area || '—' }} ha</strong>
              </div>

              <div class="info-row">
                <span>Season Context</span>
                <strong>{{ selectedClaim.damage_report?.season?.name || '—' }}</strong>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      statusFilter: '',
      loading: false,
      activeTab: 'current', // <--- Tab state management
  ...
  computed: {
    filteredClaims() {
      ...
      // 1. Filter by Active Tab (Current open season vs historical closed seasons)
      if (this.activeTab === 'current') {
        list = list.filter(claim => claim.damage_report?.season?.status === 'open')
  ...
  watch: {
    ...
    // Reset selections cleanly when swapping tabs
    activeTab() {
      this.selectedIds = []
    }
  },
  ...
  methods: {
    ...
    farmerName(claim) {
      const user = claim.damage_report?.farm?.farmer_profile?.user
    ...
    farmerContact(claim) {
      return claim.damage_report?.farm?.farmer_profile?.user?.phone_number || '—'
    },

    farmerAddress(claim) {
      return claim.damage_report?.farm?.farmer_profile?.address || '—'
    },
